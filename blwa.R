...
site_contour <- st_read(site_shps[2]) 
site_contour2 <- st_read(site_shps[1]) 
site_shore <- st_read(site_shps[3])
site_shore2 <- st_read(site_shps[6])
site_hab <- st_read(site_shps[5])
site_hab2 <- st_read(site_shps[4])

site_shore %>%
  ggplot() +
  geom_sf(col = 'red') +
  geom_sf(data = site_shore2, fill = NA, col = 'blue')
